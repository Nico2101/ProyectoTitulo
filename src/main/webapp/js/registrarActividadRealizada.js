						...
						if (!$.isEmptyObject(data)) {
							...
							for (var i = 0; i < data.length; i++) {
								...
								if (filas >= 1) {
									...
									fechaMax = fechaMax.format('YYYY-MM-DD');

									var fechaMin = moment(
											data[i].temporada.fechaInicio,
											'YYYY/MM/DD');

									fechaMin = fechaMin.format('YYYY-MM-DD');

									console.log(fechaMin);

									if (data[i].fechaEjecucionReal == null) {
										cell4.innerHTML = '<input type="date" id="'
												+ idFecha
												+ '" min="'
												+ fechaMin
												+ '" max="'
					...
					.ajax({
						...
						success : function(data) {
							...
							if (data == true) {
								...
								// Vaciar los campos del modal por si el usuario
								// quiere seguir ingresando insumos utilizados
								$('#insumo').val(-1);
								document.getElementById('divUMInsumo').style.display = 'none';
								$('#cantidadInsumo').val("");
								$('#costoInsumo').val("");
		...
		$
				.ajax({
					...
					success : function(data) {
						...
						// vaciar datatable
						oTable = $('#listaInsumosDespuesDeGuardar').dataTable();
						oTable.fnClearTable();
						if (!$.isEmptyObject(data)) {
...
function guardarInsumoEditado() {
	...
	if (idActividadInsumo > 0 && cantidad != "" && cantidad > 0 && costo != ""
			&& costo > 0) {
				...
				.ajax({
					...
					success : function(data) {
						...
						if (data == true) {
							...
							toastr.error("Error al editar el insumo utilizado");
						}
					},
					error : function(jqXHR, errorThrown) {
						alert("Error al obtener los productos");
...
function guardarDatos() {
	...
	if (rowLength > 1) {
		...
		for (i = 1; i < rowLength; i++) {
			...
			if (typeof fecha != 'undefined' && fecha != "") {
				console.log("fecha Ejecucion: " + fecha); // Imprimir Fecha
															// Ejecucion Real
				cont++;
				...
				if (oCells[7].innerHTML == 1) {
					...
					if (cantCosechada == "") {
						toastr.warning("Debe ingresar la cantidad cosechada");
						...
						arregloDatosCosecha.push(oCells[6].innerHTML);// id
																		// actividad
						arregloDatosCosecha.push(fecha);// fecha ejecucion real
						arregloDatosCosecha.push(cantCosechada);// Cantidad
																// Cosechada

					}

				} else {
					// la actividad no es cosecha por lo tanto se guarda en otro
					// arreglo
					fecha = moment(fecha, 'YYYY-MM-DD');
					fecha = fecha.format('DD-MM-YYYY');
	...
	if (cont == 0) {
		...
		if (arregloDatos.length > 0 || arregloDatosCosecha.length > 0) {
			// Recorrer nuevamente el arreglo para preguntar si agregó insumos
			// utilizados
			...
			/*
			 * //ocultar div tabla
			 * document.getElementById('divTablaRegistrarActividadRealizada').style.display =
			 * 'none';
			 * 
			 * //ocultar predio y plan
			 * document.getElementById('divPlanAsignado').style.display =
			 * 'none'; document.getElementById('divPredio').style.display =
			 * 'none';
			 * 
			 * $('#sector').val(-1);
			 */
					...
					success : function(data) {
						...
						if (!$.isEmptyObject(data)) {
							if (data.idInsumo > 0) {
								...
								// Recargar select
								// Obtener todos los insumos para cargarlos en
								// el select
